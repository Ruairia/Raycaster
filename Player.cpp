...
namespace raycaster {
    ...
    Player::Player(const Vector2D& position, const Vector2D& direction, const Vector2D& cameraPlane, const float pitch)
    : position(position), direction(direction), cameraPlane(cameraPlane), pitch(pitch) {}
